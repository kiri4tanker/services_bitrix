...
<?else:?>
    ...
    <?if($arResult["SHOW_SMS_FIELD"] == true):?>
        ...
        <form class="register__form" method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform">
            ...
            if($arResult["BACKURL"] <> ''):
                ?>
                <input class="input" type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" />
            <?
            endif;
            ?>
            <input class="input" type="hidden" name="SIGNED_DATA" value="<?=htmlspecialcharsbx($arResult["SIGNED_DATA"])?>" />
            <table>
                <tbody>
                <tr>
                    <td><?echo GetMessage("main_register_sms")?><span class="starrequired">*</span></td>
                    <td><input class="input" size="30" type="text" name="SMS_CODE" value="<?=htmlspecialcharsbx($arResult["SMS_CODE"])?>" autocomplete="off" /></td>
                </tr>
                </tbody>
                <tfoot>
                <tr>
                    <td></td>
                    <td><input class="input" type="submit" name="code_submit_button" value="<?echo GetMessage("main_register_sms_send")?>" /></td>
                </tr>
                </tfoot>
            </table>
        </form>
				...
				<span class="bx-auth-secure" title="<?echo GetMessage("AUTH_NONSECURE_NOTE")?>">
					<div class="bx-auth-secure-icon bx-auth-secure-unlock"></div>
                                    ...
                                    case "CONFIRM_PASSWORD":
                                        ?><input class="input" size="30" type="password" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" /><?
                                        break;
                                    ...
                                    case "PERSONAL_PHOTO":
                                    case "WORK_LOGO":
                                        ?><input class="input" size="30" type="file" name="REGISTER_FILES_<?=$FIELD?>" /><?
                                        break;

                                    case "PERSONAL_NOTES":
                                    case "WORK_NOTES":
                                        ?><textarea class="input" cols="30" rows="5" name="REGISTER[<?=$FIELD?>]"><?=$arResult["VALUES"][$FIELD]?></textarea><?
                                        break;
                                    default:
                                        if ($FIELD == "PERSONAL_BIRTHDAY"):?><small><?=$arResult["DATE_FORMAT"]?></small><br /><?endif;
                                        ?><input class="input" size="30" type="text" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" /><?
